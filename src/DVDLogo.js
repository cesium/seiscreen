...
const widthDVDLogo = 240.8;
const heightDVDLogo = 157.4;

export default function DVDLogo({ width, height }) {
  const [{x, xSpeed}, setX] = useState({x: getRandomNumber(0, width - widthDVDLogo),xSpeed: 1});
  const [{y, ySpeed}, setY] = useState({y: getRandomNumber(0, height - heightDVDLogo),ySpeed: 1});
  ...
  function moveDVDLogo() {
    setX(({x: old, xSpeed: oldspeed}) => {
      if (old + widthDVDLogo >= width || old <= 0) {
        setRandomColors();
    ...
    setY(({y: old, ySpeed: oldspeed}) => {
      if (old + heightDVDLogo >= height || old <= 0) {
        setRandomColors();
